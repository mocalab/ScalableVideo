...
void \hyperlink{classVLCWrapper_a5acd0eb82102874f09dacaa277a2f784}{reset\-Running\-Average} ()
...
\begin{DoxyCode}
41 \{
42     \textcolor{comment}{//call the teardown function}
43     this->\hyperlink{classVLCWrapper_a58462a6c083affe4ac09e057d2303565}{freeResources}();
44 \}
\end{DoxyCode}
...
\begin{DoxyCode}
48 \{
49     libvlc\_media\_player\_stop(m\_mp);
50     libvlc\_media\_player\_release(m\_mp);
51     libvlc\_release(m\_vlc);
52 \}
\end{DoxyCode}
...
\begin{DoxyCode}
227 \{
228     \textcolor{keywordtype}{float} avg = 0.0;
229     \textcolor{comment}{//Lock the mutex}
230     m\_avgmutex.lock();
231     avg = this->m\_bravg;
232     m\_avgmutex.unlock();
233 
234     \textcolor{keywordflow}{return} avg;
235 
236 \}
\end{DoxyCode}
...
\begin{DoxyCode}
173 \{
174     \textcolor{keywordflow}{return} m\_fps;
175 \}
\end{DoxyCode}
...
\begin{DoxyCode}
179 \{
180     libvlc\_media\_stats\_t stats;
181 
182     \textcolor{keywordtype}{int} err = libvlc\_media\_get\_stats(m\_media, &stats);
183 
184     \textcolor{keywordflow}{if}(err)
185     \{
186         \textcolor{comment}{//Compute the running average}
187 
188         \textcolor{comment}{//Lock the mutex}
189         m\_avgmutex.lock();
190         \textcolor{comment}{//Increment average counter}
191         m\_current++;
192         \textcolor{comment}{//Get the new average}
193         m\_bravg = (m\_bravg * (m\_current-1) + stats.f\_input\_bitrate) / m\_current;
194         \textcolor{comment}{//Unlock the mutex}
195         m\_avgmutex.unlock();
196 
197         \textcolor{comment}{//Print}
198         \hyperlink{Log_8h_aba6772a53f37234ac4fa7570b4d49c5a}{DEBUG}() << stats.f\_demux\_bitrate << \textcolor{stringliteral}{"    "} << stats.f\_input\_bitrate << \textcolor{stringliteral}{"    "} << m\_bravg;
199 
200     \}
201     \textcolor{keywordflow}{else}
202     \{
203 
204         \hyperlink{Log_8h_aba6772a53f37234ac4fa7570b4d49c5a}{DEBUG}() << \textcolor{stringliteral}{"FAILED TO GET STATISTICS: "} << err;
205     \}
206 
207 
208 \}
...
\begin{DoxyCode}
212 \{
213     \textcolor{keywordtype}{bool} status = \textcolor{keyword}{true};
214     \textcolor{comment}{//See if media is playing}
215     status = this->\hyperlink{classVLCWrapper_a52d4cde1548105d10d1b8906fac68dac}{isPlaying}();
216     \textcolor{comment}{//Get the statistics}
217     \textcolor{keywordflow}{if}(status)
218     \{
219        status = libvlc\_media\_get\_stats(m\_media, stats);
220     \}
221 
222     \textcolor{keywordflow}{return} status;
223 \}
\end{DoxyCode}
...
\begin{DoxyCode}
140 \{
141     \textcolor{keywordtype}{bool} playing = this->m\_isPlaying;
142     \textcolor{comment}{//Only get width and height if video is playing}
143     \textcolor{keywordflow}{if}(playing)
144     \{
145         \textcolor{comment}{//*width = libvlc\_video\_get\_width(m\_mp);}
146         \textcolor{comment}{//*height = libvlc\_video\_get\_height(m\_mp);}
147         libvlc\_video\_get\_size(m\_mp, 0, (\textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} *)width, (\textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} *)height);
148     \}
149     \textcolor{keywordflow}{return} playing;
150 \}
\end{DoxyCode}
...
24         \textcolor{stringliteral}{"--udp-caching=0"},
25         \textcolor{stringliteral}{"--h264-fps=40"}
26     \};
27 
28     \textcolor{comment}{//Instantiate vlc}
29     \textcolor{comment}{//m\_vlc = libvlc\_new(sizeof(vlc\_args) / sizeof(vlc\_args[0]), vlc\_args);}
30     m\_vlc = libvlc\_new(0, 0);
31     \textcolor{comment}{//Instantiate a media player}
32     m\_mp = libvlc\_media\_player\_new(m\_vlc);
33 
34     \textcolor{comment}{//Logging callback -- HAVE TO UPGRADE UBUNTU TO GET LATEST VERSION OF LIBVLC}
35     \textcolor{comment}{//libvlc\_log\_set(m\_vlc, log\_msg, (void *)this->m\_data);}
36 
37 
38 \}
\end{DoxyCode}
...
\begin{DoxyCode}
167 \{
168     \textcolor{keywordflow}{return} (1 == libvlc\_media\_player\_is\_playing(m\_mp));
169     \textcolor{comment}{//return this->m\_isPlaying;}
170 \}
\end{DoxyCode}
...
\begin{DoxyCode}
134 \{
135     libvlc\_media\_player\_pause(m\_mp);
136 \}
\end{DoxyCode}
...
\begin{DoxyCode}
80 \{
81     \textcolor{keywordtype}{int} status = 0;
82     \textcolor{comment}{//Set media player element}
83     libvlc\_media\_player\_set\_media(m\_mp, m\_media);
84     \textcolor{comment}{//Non-windows}
85     libvlc\_media\_player\_set\_xwindow(m\_mp, winId);
86 
87     \textcolor{comment}{//Set the media options we desire}
88     this->setMediaOptions();
89 
90     \textcolor{comment}{//Play media}
91     status = libvlc\_media\_player\_play(m\_mp);
92 
93     \textcolor{keywordflow}{if}(status != -1)
94         this->m\_isPlaying = \textcolor{keyword}{true};
95 
96     \textcolor{keywordflow}{return} status;
97 \}
\end{DoxyCode}
...
\begin{DoxyCode}
101 \{
102     \textcolor{keywordtype}{int} status = 0;
103     \textcolor{comment}{//Set media player element}
104     libvlc\_media\_player\_set\_media(m\_mp, m\_media);
105     \textcolor{comment}{//Windows specific}
106     libvlc\_media\_player\_set\_hwnd(m\_mp, winId);
107 
108     \textcolor{comment}{//Set the media options we desire}
109     this->setMediaOptions();
110 
111     \textcolor{comment}{//Play media}
112     status = libvlc\_media\_player\_play(m\_mp);
113 
114     \textcolor{keywordflow}{if}(status != -1)
115         this->m\_isPlaying = \textcolor{keyword}{true};
116 
117     \textcolor{keywordflow}{return} status;
118 \}
\end{DoxyCode}
...
\begin{DoxyCode}
153 \{
154     \textcolor{comment}{//int ret = libvlc\_media\_player\_set\_rate(m\_mp, ((float)fps) / 30);}
155     \textcolor{comment}{//Set the media options we desire}
156     this->setMediaOptions();
157 
158     \textcolor{comment}{//Set media player element}
159     libvlc\_media\_player\_set\_media(m\_mp, m\_media);
160 
161     \textcolor{comment}{//Play media}
162     \textcolor{keywordtype}{int} status = libvlc\_media\_player\_play(m\_mp);
163 \}
\end{DoxyCode}
\hypertarget{classVLCWrapper_a5acd0eb82102874f09dacaa277a2f784}{\index{V\-L\-C\-Wrapper@{V\-L\-C\-Wrapper}!reset\-Running\-Average@{reset\-Running\-Average}}
\index{reset\-Running\-Average@{reset\-Running\-Average}!VLCWrapper@{V\-L\-C\-Wrapper}}
\subsubsection[{reset\-Running\-Average}]{\setlength{\rightskip}{0pt plus 5cm}void V\-L\-C\-Wrapper\-::reset\-Running\-Average (
\begin{DoxyParamCaption}
{}
\end{DoxyParamCaption}
)}}\label{classVLCWrapper_a5acd0eb82102874f09dacaa277a2f784}
...
\begin{DoxyCode}
240 \{
241     \textcolor{comment}{//Lock the mutex}
242     m\_avgmutex.lock();
243     m\_bravg = 0.0;
244     m\_current = 0;
245     m\_avgmutex.unlock();
246 
247 \}
\end{DoxyCode}
...
\begin{DoxyCode}
56 \{
57     \textcolor{keywordflow}{if}(m\_isPlaying)
58     \{
59         \textcolor{comment}{//Stop current video}
60         this->\hyperlink{classVLCWrapper_a2ddae6b4fb05cd9cd149c6769d8fa7a9}{stop}();
61     \}
62     \textcolor{comment}{//Open the file specified in the open string}
63     this->m\_media = libvlc\_media\_new\_path(m\_vlc, file\_name);
64 \}
\end{DoxyCode}
...
\begin{DoxyCode}
68 \{
69     \textcolor{keywordflow}{if}(m\_isPlaying)
70     \{
71         \textcolor{comment}{//Stop current video}
72         this->\hyperlink{classVLCWrapper_a2ddae6b4fb05cd9cd149c6769d8fa7a9}{stop}();
73     \}
74     \textcolor{comment}{//Open specified url}
75     this->m\_media = libvlc\_media\_new\_location(m\_vlc, url);
76 \}
\end{DoxyCode}
...
\begin{DoxyCode}
121 \{
122     libvlc\_media\_player\_stop(m\_mp);
123     \textcolor{comment}{//Release the media}
124     libvlc\_media\_release(m\_media);
125     this->m\_isPlaying = \textcolor{keyword}{false};
126 \}
\end{DoxyCode}
